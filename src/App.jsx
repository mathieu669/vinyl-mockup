...
const cleanSettings = o => ({ ...o, phaseScales: [...(o.phaseScales || BASE.phaseScales)], phasePos: (o.phasePos || BASE.phasePos).map(p => [p[0], p[1]]) });
...
export default function VinylMockupAnimator() {
  ...
  const [profiles, setProfiles] = useState(() => { try { return JSON.parse(localStorage.getItem(PROFILE_KEY) || "[]"); } catch { return []; } });
  ...
  const storeProfiles = next => { setProfiles(next); try { localStorage.setItem(PROFILE_KEY, JSON.stringify(next)); } catch {} };
  const saveProfile = () => { const name = (profileName.trim() || `Profil ${profiles.length + 1}`).slice(0, 40); if (!name) return; const i = profiles.findIndex(p => p.name === name); if (i < 0 && profiles.length >= 5) return; const item = { name, settings: cleanSettings(s) }; storeProfiles(i >= 0 ? profiles.map((p, j) => j === i ? item : p) : [...profiles, item]); setProfileName(""); };
  const loadProfile = p => { setS({ ...BASE, ...cleanSettings(p.settings) }); clearExport(); };
  const deleteProfile = name => storeProfiles(profiles.filter(p => p.name !== name));
  ...
  return <div className="min-h-screen bg-[#0a0a0b] text-neutral-100"><header className="border-b border-white/10 bg-neutral-950/80 px-4 py-4 md:px-8"><div className="mx-auto flex max-w-7xl flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.28em] text-neutral-500">Vinyl product loop maker</p><h1 className="mt-1 text-2xl font-semibold md:text-3xl">Générateur de mockup vidéo vinyle</h1></div><div className="flex flex-wrap gap-2 text-xs text-neutral-400"><span className="rounded-full border border-white/10 px-3 py-1">9:16</span><span className="rounded-full border border-white/10 px-3 py-1">1080 × 1920</span><span className="rounded-full border border-white/10 px-3 py-1">{s.duration}s</span><span className="rounded-full border border-white/10 px-3 py-1">{Object.values(src).filter(Boolean).length}/5 fichiers</span></div></div></header>
    <div className="mx-auto grid max-w-7xl gap-6 p-4 md:p-8 lg:grid-cols-[440px_1fr]"><aside className="space-y-4 rounded-[2rem] bg-white p-4 text-neutral-950 shadow-2xl md:p-5">
      ...
      <Sec n="1B" title="Profils enregistrés"><div className="grid gap-3"><div className="flex gap-2"><input value={profileName} onChange={e => setProfileName(e.target.value)} maxLength={40} placeholder={profiles.length >= 5 ? "5 profils maximum" : "Nom du profil"} className="min-w-0 flex-1 rounded-xl border border-neutral-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-neutral-950 focus:ring-2 focus:ring-neutral-950/10" /><button type="button" disabled={profiles.length >= 5 && !profiles.some(p => p.name === profileName.trim())} onClick={saveProfile} className="rounded-xl bg-neutral-950 px-3 py-2 text-xs font-semibold text-white disabled:opacity-40">Enregistrer</button></div><div className="grid gap-2">{profiles.length ? profiles.map(p => <div key={p.name} className="flex items-center gap-2 rounded-2xl border border-neutral-200 bg-white p-2"><button type="button" onClick={() => loadProfile(p)} className="min-w-0 flex-1 truncate px-2 text-left text-sm font-semibold text-neutral-800">{p.name}</button><button type="button" onClick={() => { setS({ ...BASE, ...cleanSettings(p.settings) }); setProfileName(p.name); }} className="rounded-lg bg-neutral-100 px-2 py-1 text-xs font-semibold text-neutral-600">Modifier</button><button type="button" onClick={() => deleteProfile(p.name)} className="rounded-lg bg-neutral-950 px-2 py-1 text-xs font-semibold text-white">Effacer</button></div>) : <p className="text-xs text-neutral-500">Aucun profil enregistré. Les profils sont stockés localement dans ce navigateur.</p>}</div><p className="text-xs text-neutral-500">{profiles.length}/5 profils utilisés.</p></div></Sec>
